...
public class Board extends ArrayList<List<Cell>> implements Serializable {
    ...
    static final int DO_NOT_COUNT_CELLS = 2;
    ...
    ArrayList<Integer> NonExistentList = new ArrayList<Integer>();
    boolean gotNonExistentList = false;
    ...
    private ArrayList<Integer> getRandomCells(Integer[] columnHieght, int amount) {
        ...
        if(!gotNonExistentList) {
            ...
            for (int i = 0; i < amount; i++) {
                NonExistentList.add(0 + (int) (Math.random() * tottal));
            }

            boolean ifDuplicates = false;
            while (!ifDuplicates) {
                int duplicateDiff = NonExistentList.size();
                Set<Integer> hs = new HashSet<>();
                hs.addAll(NonExistentList);
                NonExistentList.clear();
                NonExistentList.addAll(hs);
                duplicateDiff = duplicateDiff - NonExistentList.size();
                if (duplicateDiff > 0) {
    ...
    public void createBoard(Integer[] columnHieght) {
        getRandomCells(columnHieght, DO_NOT_COUNT_CELLS);
